...
- The 35% wheel share remains a first-pass assumption. The current seeded 200,000-spin base-game oracle is green at 95.91%, but it does not include this human-success-dependent handoff. A read-only 1,000-session perfect-clear sample averaged 3,454.64 Glee-coins across 85.96 resolved standard rounds per success at `betPerLine = 1`. Re-run the seeded RTP, free-spin, 8+ cascade, UniGlee, Treat Time, memory-clear bounds, and bonus-frequency diagnostics with the memory bonus included before release. Do not weaken the existing 96% ±0.5 gate.
