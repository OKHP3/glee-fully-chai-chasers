...
On the required gates above: the trigger frequency is now genuinely simulated rather than inferred from the old `1/400` gate. Measured 2026-08-09 at 1 in 1,370 on the seeded oracle against a spec target of ~1 in 1,277 (`npx vitest run src/engine/simulation.test.ts --reporter=verbose`; `src/engine/uniglee.ts` lines 32 to 38). The full suite is green at 170 tests across 24 files.

The full-game fleet measures **98.70%** across 2,000,000 paid spins on seeds 1 to 40 (95% CI 97.93% to 99.47%), which is above the documented 95% to 98% band and is open as **D8** in `docs/DECISION-LOG.md`. Reproduce with `for s in $(seq 1 40); do npx tsx scripts/sim-agent.ts a$s $s 50000; done`. That figure assumes a perfect player on the two interactive bonuses, so it is a generous-play ceiling.

UniGlee is the single largest bonus contributor at **7.47 points of RTP**, captured at 1 in 1,229 across the fleet. That matters directly to D7: at the shipped 40/60/80 award UniGlee already carries more RTP than any other bonus, so raising the award to S30's 300/400/500 would multiply the largest contributor in a game that is already above band. D7 and D8 should be ruled together.
