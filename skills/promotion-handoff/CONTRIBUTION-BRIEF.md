# OKHP3/skillz Contribution Brief — Replit Family (4 Skills)
...
Four new `developer-tooling` skills extracted from the Glee-fully Chai Chasers Designathon project are ready for promotion into the `replit/` family of `OKHP3/skillz`. All four passed public-safety review, structural validation, and mirror sync. Promotion manifests are stored alongside this brief.
...
- ✅ MIT license declared on all four
- ✅ Illustrative examples use fictional project names (chai-chasers, Phoebe's Lap Quest, UniGlee) — not private identifiers
...
1. **Review this brief and the 4 manifests** — confirm the `replit/` family path and any name collision check in the live `OKHP3/skillz` repo.
2. **Authorize the PR** — the brief is a local diff only. No commit, push, or PR has been made.
3. **Run `--full` catalog** after placing files in `OKHP3/skillz` to regenerate `README.md` and `replit/FAMILY.md`.
4. **Record accepted commit hash** in each manifest's `canonical_target.accepted_commit_or_hash` field after merge.
...
Source files (publication mirrors, now complete and byte-equal with runtime sources):
